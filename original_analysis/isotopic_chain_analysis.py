...
def barrier_exit_radius(a, Z1, l, mu, Qa):
    ...
    denom = disc - V0
    return (1.0/a) * np.log(num/denom + 1.0)
...
def barrier_integral(Rt, Rout, a, V0, V1, Qa, l):
    def F(rv): return (1.0/a) * (I1(rv, a, V0, V1, Qa) + I2(rv, a, V0, V1, Qa))
    return F(Rout) - F(Rt)
...
def half_life_log10(A_parent, Z_parent, Qa, a, l=0, h=0):
    A1  = A_parent - 4
    Rt  = nuclear_radius(A1) + nuclear_radius(4)
    mu  = reduced_mass(A1)
    xl  = bessel_zero(l)
    Z1  = Z_parent - 2
...
# ─── Extract individual a for each nucleus ────────────────────────────────────
results = []
...
for Z_key, chain in all_nuclei.items():
    ...
    for entry in chain["nuclei"]:
        ...
        def residual(a, A=A, Z=Z, Qa=Qa, l=l, h=h, logT_exp=logT_exp):
            return half_life_log10(A, Z, Qa, a, l, h) - logT_exp
        try:
            a_opt = brentq(residual, 1e-12, 1e-2, xtol=1e-25, maxiter=1000)
            solved = True
        except:
            res = minimize_scalar(
                lambda a, A=A, Z=Z, Qa=Qa, l=l, h=h, logT_exp=logT_exp: abs(residual(a)),
                bounds=(1e-12, 1e-2),
                method='bounded',
                options={'xatol': 1e-25}
            )
        ...
        N = A - Z
        results.append((N, Z, a_opt, l, solved))
        status = "exact" if solved else "minimized"
        print(f"A={A:3d} Z={Z:2d} N={N:3d}  a={a_opt:.6e}  l={l}  ({status})")
...
# ─── Plot function ────────────────────────────────────────────────────────────
def plot_chain(Z0, results, all_nuclei):
    name   = chain_names[Z0]
    lookup = {(r[0], r[1]): (r[2], r[3]) for r in results}

    exp_data = all_nuclei[Z0]["exp"]
    chain = [(A, Z, Qa, pairing, l, h, exp_data[A])
             for A, Z, Qa, pairing, l, h in all_nuclei[Z0]["nuclei"]
             if A in exp_data]
    ...
    for A, Z, Qa, pairing, l, h, logT_exp in chain:
        N   = A - Z
        key = (N, Z)
        if key not in lookup:
            continue
        a_opt, l_val = lookup[key]
        log_a   = np.log10(a_opt)
        favored = (l_val == 0)

        if N % 2 == 0:
            even_N_data.append((N, log_a, favored))
        else:
            odd_N_data.append((N, log_a, favored))

    even_N_data.sort(key=lambda x: x[0])
    odd_N_data.sort( key=lambda x: x[0])
    ...
    def plot_group(data, color):
        ...
        log_as = [d[1] for d in data]
      
        ax.plot(Ns, log_as, color=color, linewidth=1.2, zorder=1)

        for N_pt, log_a_pt, favored in data:
            marker = 'o' if favored else 's'
            ax.plot(N_pt, log_a_pt, marker=marker, color=color,
                    markersize=7, zorder=2)

    plot_group(even_N_data, 'black')
    plot_group(odd_N_data,  'red')

    all_N = [d[0] for d in even_N_data + odd_N_data]
    if all_N and min(all_N) <= 126 <= max(all_N):
        ax.axvline(x=126, color='gray', lw=1.2, ls='--')
        ylo, yhi = ax.get_ylim()
        ax.text(126.3, ylo + 0.92*(yhi - ylo),
                'N=126', fontsize=10, style='italic')

    handles = [
        mlines.Line2D([], [], color='black', marker='o', markersize=7,
                      linewidth=1.2, label='even N, favored ($l=0$)'),
        mlines.Line2D([], [], color='black', marker='s', markersize=7,
                      linewidth=1.2, label='even N, unfavored ($l>0$)'),
        mlines.Line2D([], [], color='red',   marker='o', markersize=7,
                      linewidth=1.2, label='odd N, favored ($l=0$)'),
        mlines.Line2D([], [], color='red',   marker='s', markersize=7,
                      linewidth=1.2, label='odd N, unfavored ($l>0$)'),
    ]
    ax.legend(handles=handles, fontsize=10, loc='lower right')

    ax.text(0.97, 0.08, f'$^{{Z0}}${name}',
            transform=ax.transAxes, fontsize=13,
            ha='right', va='bottom')
    ax.set_xlabel('N', fontsize=13)
    ax.set_ylabel(r'$\log_{10}(a)$', fontsize=13)
    ax.set_title(f'{name} isotopes (Z={Z0})', fontsize=13)
    ax.grid(True, alpha=0.25)
    plt.tight_layout()
    plt.show()

for Z0 in sorted(chain_names.keys()):
    plot_chain(Z0, results, all_nuclei)