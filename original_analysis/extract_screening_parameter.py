...
l_vals = [r[3] for r in results]

fav_idx   = [i for i, r in enumerate(results) if r[3] == 0]
unfav_idx = [i for i, r in enumerate(results) if r[3] >  0]
...
sc1 = ax.scatter(
    [N_vals[i] for i in fav_idx],
    [Z_vals[i] for i in fav_idx],
    c=[a_vals[i] for i in fav_idx],
    cmap='coolwarm', marker='o',
    vmin=vmin, vmax=vmax,
    label='favored', s=50, zorder=3)

sc2 = ax.scatter(
    [N_vals[i] for i in unfav_idx],
    [Z_vals[i] for i in unfav_idx],
    c=[a_vals[i] for i in unfav_idx],
    cmap='coolwarm', marker='D',
    vmin=vmin, vmax=vmax,
    label='unfavored', s=50, zorder=3)

plt.colorbar(sc1, ax=ax, label=r'$a$ [fm$^{-1}$]')


ax.axvline(x=126, color='steelblue', linewidth=1.5, linestyle='-',  label='N=126')
ax.axvline(x=162, color='steelblue', linewidth=1.5, linestyle='--', label='N=162')
ax.axhline(y=108, color='steelblue', linewidth=1.5, linestyle='--', label='Z=108')

ax.set_xlabel('N', fontsize=13)
ax.set_ylabel('Z', fontsize=13)
ax.set_title('Carte Z-N du paramètre d\'écrantage $a$', fontsize=13)
...
for Z_key, chain in all_nuclei.items():
    ...
    for entry in chain["nuclei"]:
        ...
        if A not in exp_data:
            continue
        key = (A, Z)
        if key in beta2_data:
            N, Z_r, a_opt, l_r, solved = results[idx]
            a_lookup[key] = a_opt
        idx += 1
...
for (A, Z), b2 in beta2_data.items():
    if (A, Z) in a_lookup:
        xs.append(b2)
        ys.append(a_lookup[(A, Z)] * 1e4)

ax.scatter(xs, ys, color='red', marker='o', s=55, zorder=3)
