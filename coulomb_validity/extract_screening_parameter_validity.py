...
- Generates Z-N maps of valid nuclei.
...
# ─── Coulomb validity check ──────────────────────────────────────────
print("Checking Coulomb validity and solving for individual a ...")
...
for Z_key, chain in all_nuclei.items():
    ...
    for entry in chain["nuclei"]:
        ...
        status = ("valid  | a={:.3e}".format(a_opt)) if valid else "INVALID (Coulomb < exp)"
        print(f"  A={A:3d} Z={Z} N={N:3d}  {status}")

# ─── Fig 3 — Z-N chart of screening parameter a ──────────────────────────────
fig, ax = plt.subplots(figsize=(12, 8))

vmin, vmax = 0, 27e-4


valid_fav   = [r for r in records if r["valid"] and r["l"] == 0]
valid_unfav = [r for r in records if r["valid"] and r["l"] >  0]

invalid_fav   = [r for r in records if not r["valid"] and r["l"] == 0]
invalid_unfav = [r for r in records if not r["valid"] and r["l"] >  0]

def scatter_valid(ax, recs, marker, label):
    if not recs:
        return None
    sc = ax.scatter(
        [r["N"] for r in recs], [r["Z"] for r in recs],
        c=[r["a_opt"] for r in recs],
        cmap='coolwarm', marker=marker,
        vmin=vmin, vmax=vmax, s=55, zorder=3, label=label)
    return sc

def scatter_invalid(ax, recs, marker, label):
    if not recs:
        return
    ax.scatter(
        [r["N"] for r in recs], [r["Z"] for r in recs],
        marker=marker, facecolors='black', edgecolors='black',
        linewidths=1.2, s=60, zorder=4, label=label)

sc = scatter_valid(ax, valid_fav,   'o', 'favored (valid)')
scatter_valid(ax, valid_unfav, 'D', 'unfavored (valid)')
scatter_invalid(ax, invalid_fav,   'o', 'favored (Coulomb < exp)')
scatter_invalid(ax, invalid_unfav, 'D', 'unfavored (Coulomb < exp)')

if sc:
    plt.colorbar(sc, ax=ax, label=r'$a\ [\mathrm{fm}^{-1}]$')

ax.axvline(x=126, color='steelblue', lw=1.5, ls='-',  label='N=126')
ax.axvline(x=162, color='steelblue', lw=1.5, ls='--', label='N=162')
ax.axhline(y=108, color='steelblue', lw=1.5, ls='--', label='Z=108')

ax.set_xlabel('N', fontsize=13)
ax.set_ylabel('Z', fontsize=13)
ax.set_title('Z–N chart of screening parameter $a$\n', fontsize=12)
...
# ─── Fig 4 — a vs β₂  (valid even-even only) ─────────────────────────────────
beta2_data = {
    (194, 84): 0.178,
    (200, 84): 0.104,
    (210, 84): 0.0141,
...
a_lookup = {(r["A"], r["Z"]): r for r in records}
...
for (A, Z), b2 in beta2_data.items():
    if (A, Z) not in a_lookup:
        continue
    rec = a_lookup[(A, Z)]
    if rec["valid"]:
        xs_valid.append(b2)
        ys_valid.append(rec["a_opt"] * 1e4)
    else:
        xs_invalid.append(b2)
        ys_invalid.append(b2)   


ax.scatter(xs_valid, ys_valid,
           color='red', marker='o', s=55, zorder=3, label='valid (Coulomb ≥ exp)')


for b2 in xs_invalid:
    ax.axvline(x=b2, color='gray', lw=0.5, ls=':', alpha=0.5)
if xs_invalid:
    ax.scatter(xs_invalid, [0]*len(xs_invalid),
               color='black', marker='x', s=70, linewidths=1.8,
               zorder=4, label='invalid (Coulomb < exp)')

ax.set_xlim(0, 0.36)
ax.set_ylim(bottom=0)
ax.set_xlabel(r'$\beta_2$', fontsize=14)
ax.set_ylabel(r'$a\ [10^{-4}\ \mathrm{fm}^{-1}]$', fontsize=14)
ax.set_title(r'Screening parameter $a$ vs quadrupole deformation $\beta_2$', fontsize=11)
