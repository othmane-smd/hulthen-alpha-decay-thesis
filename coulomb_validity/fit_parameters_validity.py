...
# ─── Coulomb ──────────────────────────────────────────────────────────────────
def coulomb_barrier_exit_radius(Z1, l, mu, Qa):
    A_c=2.0*EP2*Z1; B_c=(HBAR_C**2*(l+0.5)**2)/(2.0*mu)
    return (A_c+np.sqrt(A_c**2+4*Qa*B_c))/(2*Qa)

def coulomb_wkb_integral(Rt, Rout, Z1, l, mu, Qa):
    A_c=2.0*EP2*Z1; B_c=(HBAR_C**2*(l+0.5)**2)/(2.0*mu)
    if l==0:
        u=np.clip(Rt/(A_c/Qa),0,1)
        return (A_c/np.sqrt(Qa))*(np.arccos(np.sqrt(u))-np.sqrt(u*(1-u)))
    pts,wts=np.polynomial.legendre.leggauss(500)
    mid=0.5*(Rout+Rt); half=0.5*(Rout-Rt); rs=mid+half*pts
    vals=np.array([np.sqrt(max(A_c/r+B_c/r**2-Qa,0)) for r in rs])
    return half*np.dot(wts,vals)

def half_life_coulomb(A_parent, Z_parent, Qa, l=0, h=0):
    A1=A_parent-4; Rt=nuclear_radius(A1)+nuclear_radius(4)
    mu=reduced_mass(A1); xl=bessel_zero(l); Z1=Z_parent-2
    nu=(xl*HBAR_C*3e23)/(2.0*mu*Rt**2)
    Rout=coulomb_barrier_exit_radius(Z1,l,mu,Qa)
    if Rout<=Rt: return np.nan
    I_val=coulomb_wkb_integral(Rt,Rout,Z1,l,mu,Qa)
    ln_P=-(2.0/HBAR_C)*np.sqrt(2.0*mu)*I_val
    return np.log10(np.log(2))+h-np.log10(nu)-(ln_P/np.log(10))
...
# ─── Validity check & group building ─────────────────────────────────────────
print("="*65)
print("COULOMB VALIDITY CHECK — ALL CHAINS")
print("="*65)

all_unfav_members = defaultdict(list)

for Z, chain in all_nuclei.items():
    exp_dict = chain["exp"]
    for A,Z_,Qa,pairing,l,h_tuple in chain["nuclei"]:
        if A not in exp_dict: continue
        if l == 0: continue
        logT_exp = exp_dict[A]
        all_unfav_members[pairing].append((A,Z_,Qa,pairing,l,0,logT_exp))

# ─── Fiting h from Coulomb on ALL unfavored nuclei ──────────────────────
def sigma_coulomb_unfav(h_val, members):
    sq = []
    for A,Z_,Qa,pairing,l,h_fixed,logT_exp in members:
        try:
            logT_th = half_life_coulomb(A,Z_,Qa,l,h_val)
...
for pairing in ["ee","eo","oe","oo"]:
    fitted_h[pairing + "_unfav"] = 0.0

print("  FAVORED: h = 0 (fixed by definition)\n")
print("  UNFAVORED — fitting h via Coulomb:")
for pairing in ["eo","oe","oo"]:
    members = all_unfav_members.get(pairing, [])
    if not members:
        print(f"    {pairing}: no nuclei")
        continue
    res = minimize_scalar(
            lambda h, m=members: sigma_coulomb_unfav(h, m),
            bounds=(0, 5), method='bounded')
    fitted_h[pairing + "_unfav"] = res.x
    s = sigma_coulomb_unfav(res.x, members)
    print(f"    {pairing}: n={len(members):3d}  h={res.x:.4f}   σ_Coulomb={s:.4f}")

# ─── groups with correct validity filter ────────────────────────────────
...
for Z, chain in all_nuclei.items():
    ...
    for A, Z_, Qa, pairing, l, h_tuple in chain["nuclei"]:
        ...
        if is_favored:
            logT_C   = half_life_coulomb(A, Z_, Qa, l=0, h=0)
            is_valid = np.isfinite(logT_C) and (logT_C >= logT_exp)

            if is_valid:
                groups[(pairing, "fav")].append(
                    (A, Z_, Qa, pairing, l, 0.0, logT_exp)
                )
            else:
                invalid_fav.append((name, A, Z_, pairing, logT_C, logT_exp))

        else:
            h_fitted = fitted_h.get(pairing + "_unfav", 0.0)
            logT_C   = half_life_coulomb(A, Z_, Qa, l, h=h_fitted)
            is_valid = np.isfinite(logT_C) and (logT_C >= logT_exp)

            if is_valid:
                groups[(pairing, "unfav")].append(
                    (A, Z_, Qa, pairing, l, h_fitted, logT_exp)
                )
            else:
                invalid_unfav.append((name, A, Z_, pairing, l,
                                      logT_C, logT_exp, h_fitted))


# ─── Fit a using Hulthen with fixed h ────────────────────────────────
def sigma_hulthen_fav(a_val, members):
    sq = []
    for A,Z_,Qa,pairing,l,h_fixed,logT_exp in members:
        try:
            logT_th = half_life_hulthen(A,Z_,Qa,a_val,l,0)
            if np.isfinite(logT_th):
                sq.append((logT_th - logT_exp)**2)
        except:
            pass
...
def sigma_hulthen_unfav(a_val, h_fixed, members):
    sq = []
    for A,Z_,Qa,pairing,l,h_tuple,logT_exp in members:
        try:
            logT_th = half_life_hulthen(A,Z_,Qa,a_val,l,h_fixed)
            if np.isfinite(logT_th):
                sq.append((logT_th - logT_exp)**2)
        except:
            pass
...
print("\n" + "="*65)
print(" FIT a FROM HULTHEN WITH FIXED h")
print("="*65)
...
for pairing in ["ee","eo","oe","oo"]:
    ...
    if not members:
        print(f"    {pairing}: no valid nuclei")
        continue
    res    = minimize_scalar(
                lambda la, m=members: sigma_hulthen_fav(10**la, m),
                bounds=(-10, -2), method='bounded')
    a_best = 10**res.x
    s_best = sigma_hulthen_fav(a_best, members)
    best_a[key]     = a_best
    best_h[key]     = 0.0
    best_sigma[key] = s_best
    print(f"    {pairing}: n={len(members):3d}  "
          f"a={a_best:.6e} fm⁻¹   h=0 (fixed)   σ={s_best:.4f}")

print("\n  UNFAVORED (h from Coulomb fit, a fitted):")
for pairing in ["eo","oe","oo"]:
    key     = (pairing, "unfav")
    members = groups.get(key, [])
    if not members:
        print(f"    {pairing}: no valid nuclei")
        continue
    h_fixed = fitted_h[pairing + "_unfav"]
    res     = minimize_scalar(
                lambda la, m=members, hf=h_fixed: sigma_hulthen_unfav(10**la, hf, m),
                bounds=(-10, -2), method='bounded')
    a_best  = 10**res.x
    s_best  = sigma_hulthen_unfav(a_best, h_fixed, members)
    best_a[key]     = a_best
    best_h[key]     = h_fixed
    best_sigma[key] = s_best
    print(f"    {pairing}: n={len(members):3d}  "
          f"a={a_best:.6e} fm⁻¹   h={h_fixed:.4f}   σ={s_best:.4f}")

# ─────────────────── Plot  ──────────────────────────────────────────────
all_pairings_fav   = ["ee","eo","oe","oo"]
all_pairings_unfav = ["eo","oe","oo"]
...
for Z, chain in all_nuclei.items():
    ...
    A_fav_inv,   logT_fav_inv   = [], []
    A_unfav_inv, logT_unfav_inv = [], []
    A_C_all,     logT_C_all     = [], []
    ...
    for A,Z_,Qa,pairing,l,h in nuclei_list:
        if A not in exp_dict:
            continue
        logT_exp = exp_dict[A]
        h_check  = 0.0 if l == 0 else fitted_h.get(pairing + "_unfav", 0.0)
        logT_C   = half_life_coulomb(A, Z_, Qa, l, h_check)
        A_C_all.append(A); logT_C_all.append(logT_C)

        is_valid = np.isfinite(logT_C) and (logT_C >= logT_exp)
        key      = (pairing, "fav" if l==0 else "unfav")
        a_use    = best_a.get(key)
        h_use   = best_h.get(key, 0.0)
        logT_th = half_life_hulthen(A,Z_,Qa,a_use,l,h_use) if a_use else np.nan

        if l == 0:
            if is_valid: A_fav_v.append(A);   logT_fav_v.append(logT_th)
            else:        A_fav_inv.append(A); logT_fav_inv.append(logT_th)
        else:
            if is_valid: A_unfav_v.append(A);   logT_unfav_v.append(logT_th)
            else:        A_unfav_inv.append(A); logT_unfav_inv.append(logT_th)

    # Build σ annotation lines
    sig_lines = []
    for pairing in all_pairings_fav:
        k = (pairing,"fav")
        if k in best_sigma:
            sig_lines.append(f"fav {pairing}: a={best_a[k]:.2e}  σ={best_sigma[k]:.3f}")
    for pairing in all_pairings_unfav:
        k = (pairing,"unfav")
        if k in best_sigma:
            sig_lines.append(f"unfav {pairing}: a={best_a[k]:.2e}  σ={best_sigma[k]:.3f}")

    fig, ax = plt.subplots(figsize=(11, 6))

    ax.plot(A_exp_s, logT_exp_s, 'k-', lw=1.8, label="Exp", zorder=5)
    ax.scatter(A_C_all, logT_C_all, marker='^', s=28, color='darkorange',
               alpha=0.75, zorder=3, label="Coulomb (upper bound)")
    if A_fav_inv:
        ax.scatter(A_fav_inv, logT_fav_inv, marker='x', s=55, color='lightcoral',
                   linewidths=1.5, zorder=2, label="Th. fav — invalid")
    if A_unfav_inv:
        ax.scatter(A_unfav_inv, logT_unfav_inv, marker='x', s=55, color='lightskyblue',
                   linewidths=1.5, zorder=2, label="Th. unfav — invalid")
    if A_fav_v:
        ax.scatter(A_fav_v, logT_fav_v, marker='o', s=45, color='red',
                   zorder=4, label="Th. fav — valid")
    if A_unfav_v:
        ax.scatter(A_unfav_v, logT_unfav_v, marker='o', s=45, color='blue',
                   zorder=4, label="Th. unfav — valid")

    A_vals  = [A for A,*_ in nuclei_list]
    N126_A  = 126 + Z
    if min(A_vals) <= N126_A <= max(A_vals):
        ax.axvline(x=N126_A, color='gray', ls='--', alpha=0.5, label="N=126")

    ax.set_xlabel("A", fontsize=12)
    ax.set_ylabel(r"$\log_{10}(T_{1/2})$ [s]", fontsize=12)
    sig_str = "  |  ".join(sig_lines[:4])
    ax.set_title(
        f"$^{{}}${name} (Z={Z}) isotopic chain — α decay half-lives"
    )
...
for Z, chain in all_nuclei.items():
    name = chain_names[Z]
    exp_dict = chain["exp"]
    for A, Z_, Qa, pairing, l, h in chain["nuclei"]:
        if A not in exp_dict:
            continue
        logT_exp = exp_dict[A]
        h_check  = 0.0 if l == 0 else fitted_h.get(pairing + "_unfav", 0.0)
        ...
        fav_key  = "fav" if l == 0 else "unfav"
        key      = (pairing, fav_key)
        a_use    = best_a.get(key, np.nan)
        h_use   = best_h.get(key, 0.0)
        logT_th = half_life_hulthen(A,Z_,Qa,a_use,l,h_use) if np.isfinite(a_use) else np.nan
        ...
        rows.append({
            ...
            "fav":     fav_key,
            "logT_exp": logT_exp,
            "logT_Coulomb": logT_C if not np.isnan(logT_C) else "",
            "logT_Hulthen": logT_th if np.isfinite(logT_th) else "",
