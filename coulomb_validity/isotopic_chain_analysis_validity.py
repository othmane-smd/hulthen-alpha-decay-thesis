...
def barrier_exit_radius(a, Z1, l, mu, Qa):
...
def barrier_integral(Rt, Rout, a, V0, V1, Qa):
...
def half_life_hulthen(A_parent, Z_parent, Qa, a, l=0, h=0):
    ...
    V0,V1=V0_V1(a,Z1,l,mu)
    Rout=barrier_exit_radius(a,Z1,l,mu,Qa)
    I_val=barrier_integral(Rt,Rout,a,V0,V1,Qa)
    ln_P=-(2.0/HBAR_C)*np.sqrt(2.0*mu)*I_val
    return np.log10(np.log(2))+h-np.log10(nu)-(ln_P/np.log(10))

def coulomb_exit_radius(Z1, l, mu, Qa):
...
def half_life_coulomb(A_parent, Z_parent, Qa, l=0, h=0):
    ...
    mu=reduced_mass(A1); xl=bessel_zero(l); Z1=Z_parent-2
    nu=(xl*HBAR_C*3e23)/(2.0*mu*Rt**2)
    Rout=coulomb_exit_radius(Z1,l,mu,Qa)
...
# ─── Hindrance factors ────────────────────────────────────────────────────────
h_unfav = {"eo": 2.0119, "oe": 1.6182, "oo": 1.8394}
...
# ─── Solve for a per nucleus ──────────────────────────────────────────────────
print("Solving for individual a per nucleus ...")
records = []   

for Z_key, chain in all_nuclei.items():
    exp_data = chain["exp"]
    for entry in chain["nuclei"]:
        A, Z, Qa, pairing, l, h = entry
        if A not in exp_data:
            continue
        logT_exp = exp_data[A]
        N = A - Z

        logT_C = half_life_coulomb(A, Z, Qa, l, h)
        valid  = (not np.isnan(logT_C)) and (logT_C >= logT_exp)
        ...
        if valid:
            def residual(a):
                return half_life_hulthen(A, Z, Qa, a, l, h) - logT_exp
            try:
                a_opt  = brentq(residual, 1e-12, 1e-2, xtol=1e-25, maxiter=1000)
                solved = True
            except Exception:
                res    = minimize_scalar(lambda a: abs(residual(a)),
                                        bounds=(1e-12, 1e-2), method='bounded',
                                        options={'xatol': 1e-25})
                a_opt  = res.x
                solved = False
        ...
        records.append({
            "A": A, "Z": Z, "N": N, "Qa": Qa,
            "pairing": pairing, "l": l, "h": h,
            "logT_exp": logT_exp, "logT_C": logT_C,
            "valid": valid, "a_opt": a_opt, "solved": solved,
        })

print(f"Done. {sum(r['valid'] for r in records)} valid / {len(records)} total")

# ─── Plot settings matching the reference figure ─────────────────────────────

chain_info = {
    84: ("Po", 84), 85: ("At", 85), 86: ("Rn", 86), 87: ("Fr", 87),
    88: ("Ra", 88), 89: ("Ac", 89), 90: ("Th", 90), 91: ("Pa", 91), 92: ("U",  92),
    93: ("Np",  93), 94: ("Pu",  94), 95: ("Am",  95), 96: ("Cm",  96), 97: ("Bk",  97),
    98: ("Cf",  98), 99: ("Es",  99), 100: ("Fm",  100), 101: ("Md",  101), 102: ("No",  102),
    104: ("Rf",  104), 106: ("Sg",  106), 108: ("Hs",  108), 110: ("Ds",  110), 114: ("Fl",  114),
    116: ("Lv",  116),
}

INVALID_Y_FRAC = 0.04

for Z, (name, _) in chain_info.items():
    chain = [r for r in records if r["Z"] == Z]
    if not chain:
        continue

    even_fav_valid   = [r for r in chain if r["N"]%2==0 and r["l"]==0 and r["valid"]]
    even_unfav_valid = [r for r in chain if r["N"]%2==0 and r["l"]>0  and r["valid"]]
    odd_fav_valid    = [r for r in chain if r["N"]%2==1 and r["l"]==0 and r["valid"]]
    odd_unfav_valid  = [r for r in chain if r["N"]%2==1 and r["l"]>0  and r["valid"]]

    invalid_even_fav   = [r for r in chain if r["N"]%2==0 and r["l"]==0 and not r["valid"]]
    invalid_even_unfav = [r for r in chain if r["N"]%2==0 and r["l"]>0  and not r["valid"]]
    invalid_odd_fav    = [r for r in chain if r["N"]%2==1 and r["l"]==0 and not r["valid"]]
    invalid_odd_unf    = [r for r in chain if r["N"]%2==1 and r["l"]>0  and not r["valid"]]

    fig, ax = plt.subplots(figsize=(11, 6.5))

    all_even_valid = sorted(even_fav_valid + even_unfav_valid, key=lambda r: r["N"])
    if all_even_valid:
        Ns = [r["N"] for r in all_even_valid]
        ys = [np.log10(r["a_opt"]) for r in all_even_valid]
        ax.plot(Ns, ys, color='black', lw=1.2, zorder=3)
        if even_fav_valid:
            recs_s = sorted(even_fav_valid, key=lambda r: r["N"])
            ax.scatter([r["N"] for r in recs_s],
                       [np.log10(r["a_opt"]) for r in recs_s],
                       color='black', marker='o', s=55, zorder=4,
                       label='Even N, favored (l=0)')
        if even_unfav_valid:
            recs_s = sorted(even_unfav_valid, key=lambda r: r["N"])
            ax.scatter([r["N"] for r in recs_s],
                       [np.log10(r["a_opt"]) for r in recs_s],
                       color='black', marker='s', s=55, zorder=4,
                       label='Even N, unfavored (l>0)')

    all_odd_valid = sorted(odd_fav_valid + odd_unfav_valid, key=lambda r: r["N"])
    if all_odd_valid:
        Ns = [r["N"] for r in all_odd_valid]
        ys = [np.log10(r["a_opt"]) for r in all_odd_valid]
        ax.plot(Ns, ys, color='red', lw=1.2, zorder=3)
        if odd_fav_valid:
            recs_s = sorted(odd_fav_valid, key=lambda r: r["N"])
            ax.scatter([r["N"] for r in recs_s],
                       [np.log10(r["a_opt"]) for r in recs_s],
                       color='red', marker='o', s=55, zorder=4,
                       label='Odd N, favored (l=0)')
        if odd_unfav_valid:
            recs_s = sorted(odd_unfav_valid, key=lambda r: r["N"])
            ax.scatter([r["N"] for r in recs_s],
                       [np.log10(r["a_opt"]) for r in recs_s],
                       color='red', marker='s', s=55, zorder=4,
                       label='Odd N, unfavored (l>0)')

    all_N = [r["N"] for r in chain]
    if min(all_N) <= 126 <= max(all_N):
        ax.axvline(x=126, color='steelblue', lw=1.5, ls='--',
                   alpha=0.8, label='N = 126', zorder=2)

    ax.set_xlabel('Neutron number N', fontsize=13)
    ax.set_ylabel(r'$\log_{10}(a)$', fontsize=13)
    ax.set_title(f'{name} isotopes (Z = {Z}): $\\log_{{10}}(a)$ vs N', fontsize=13)
    ax.grid(True, alpha=0.3)

    all_invalid = invalid_even_fav + invalid_even_unfav + invalid_odd_fav + invalid_odd_unf
    if all_invalid:
        ymin, ymax = ax.get_ylim()
        y_cross = ymin + 0.04*(ymax - ymin)

        for recs, color, marker, label in [
            (invalid_even_fav,   'black',   'x', 'Invalid even N fav (Coulomb < exp)'),
            (invalid_even_unfav, 'grey',   'x', 'Invalid even N unfav (Coulomb < exp)'),
            (invalid_odd_fav,    'red',     'x', 'Invalid odd N fav (Coulomb < exp)'),
            (invalid_odd_unf,    'darkred', 'x', 'Invalid odd N unfav (Coulomb < exp)'),
        ]:
            if recs:
                ax.scatter([r["N"] for r in recs], [y_cross]*len(recs),
                           marker=marker, color=color, s=90, linewidths=2,
